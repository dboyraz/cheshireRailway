...
const UserSetupSimple = () => {
    ...
    const loadOrgs = async () => {
      try {
        const response = await fetch(
          "https://serverrailway-production-e3c3.up.railway.app/api/user/organizations"
        );
  ...
  useEffect(() => {
    if (formData.unique_id.length > 0) {
      const timer = setTimeout(async () => {
        ...
        try {
          const response = await fetch(
            `https://serverrailway-production-e3c3.up.railway.app/api/user/unique-id/check?id=${formData.unique_id}`
          );
          const data = await response.json();
  ...
  useEffect(() => {
    if (formData.organization_id.length > 0) {
      const timer = setTimeout(async () => {
        ...
        try {
          const response = await fetch(
            `https://serverrailway-production-e3c3.up.railway.app/api/user/organization/check?id=${formData.organization_id}`
          );
          const data = await response.json();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiFetch("https://serverrailway-production-e3c3.up.railway.app/api/user/create", {
        method: "POST",
