...
export const useUserProfile = () => {
  ...
  const loadOrganizations = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch('https://serverrailway-production-e3c3.up.railway.app/api/user/organizations');
  ...
  const checkUniqueIdAvailability = useCallback(async (uniqueId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://serverrailway-production-e3c3.up.railway.app/api/user/unique-id/check?id=${encodeURIComponent(uniqueId)}`
      );
      const data = await response.json();
  ...
  const checkOrganizationExists = useCallback(async (organizationId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://serverrailway-production-e3c3.up.railway.app/api/user/organization/check?id=${encodeURIComponent(organizationId)}`
      );
      const data = await response.json();
  ...
  const createUserProfile = useCallback(async (profileData: {
    ...
    try {
      ...
      const response = await fetch('https://serverrailway-production-e3c3.up.railway.app/api/user/create', {
        method: 'POST',
        headers: {
  ...
  const getUserProfile = useCallback(async () => {
    try {
      const response = await fetch('https://serverrailway-production-e3c3.up.railway.app/api/user/profile', {
        credentials: 'include', // Include auth cookies
      });
  ...
  const checkUserExists = useCallback(async (walletAddress: string) => {
    try {
      const response = await fetch(
        `https://serverrailway-production-e3c3.up.railway.app/api/user/exists?address=${encodeURIComponent(walletAddress)}`
      );
      const data = await response.json();
