...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  checkAuthStatus: async (forceCheck = false) => {
    ...
    try {
      ...
      const response = await fetch('https://serverrailway-production-e3c3.up.railway.app/api/me', {
        credentials: 'include',
  ...
  checkUserExists: async (forceCheck = false) => {
    ...
    try {
      const response = await fetch(
        `https://serverrailway-production-e3c3.up.railway.app/api/user/exists?address=${encodeURIComponent(state.userAddress)}`,
        { credentials: 'include' }
      );
  ...
  logout: async () => {
    try {
      ...
      await fetch('https://serverrailway-production-e3c3.up.railway.app/api/logout', {
        method: 'POST',
        credentials: 'include',
        signal: controller.signal
      });
