...
export const useSupabaseAuthStore = create<AuthState>((set, get) => ({
  ...
  checkUserStatus: async (walletAddress: string) => {
    ...
    try {
      const response = await fetch(
        `https://serverrailway-production-e3c3.up.railway.app/api/auth/check-user?address=${encodeURIComponent(walletAddress)}`
      );
  ...
  signIn: async (walletAddress: string, message: string, signature: string) => {
    ...
    try {
      const response = await fetch('https://serverrailway-production-e3c3.up.railway.app/api/auth/signin', {
        method: 'POST',
        headers: {
  ...
  logout: async () => {
    ...
    try {
      if (jwtToken) {
        await fetch('https://serverrailway-production-e3c3.up.railway.app/api/auth/signout', {
          method: 'POST',
          headers: {
